...
export default function EnquiryForm() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${BASE_URL}/api/queries/enquiry`, {
        method: 'POST',
    ...
    <div className="w-full bg-[#2F4156] flex items-center justify-center p-6 py-8">
      <div className="bg-white rounded-2xl shadow-lg px-8 py-8 w-full max-w-2xl">
        ...
        <div className="text-center mb-6">
          <p className="text-sm font-semibold text-[#020318] uppercase tracking-widest mb-1">Sukhi Travels</p>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-3 bg-[#567C8D] hover:bg-[#2F4156] disabled:opacity-60 text-white rounded-lg text-sm font-semibold transition-colors"
          >
            {loading ? 'Submitting...' : 'Get My Travel Quote 🚀'}
          </button>
